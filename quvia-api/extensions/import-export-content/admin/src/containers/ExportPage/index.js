...
function ImportPage({ contentTypes }) {
  ...
  setTimeout(() => {
    const email = contentTypes.find(ct => ct.apiID === 'emails');
    if (email) {
      setSourceExports(email.uid);
      setTarget(email);
    }
  }, 100)


  // setSourceExports('application::emails.emails');
  // setTarget(contentTypes.find(ct => ct.apiID === 'emails'));

  const sourceOptions = useMemo(
    () =>
      [{ label: "Emails", value: "" }].concat(
...
ImportPage.componentDidMount = function() {
